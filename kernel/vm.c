...
/* kvmmappgtbl - set the mapping for a kernel page table PAGETABLE */
static void kvmmappgtbl(pagetable_t pagetable) {
  // uart registers
...
/* kvminitpgtbl - alloc a kernel page and return */
pagetable_t kvminitpgtbl(void) {
 ...
 * P.S. do not free the physical memory. */
void freepagetable(pagetable_t page) {
  pte_t pte;

  for (int i = 0; i < PGSIZE / sizeof(pte_t); i += 1) {
    pte = page[i];
...
void vmprint_helpler(pagetable_t, int);

/* vmprint - print the info about the page table PAGETABLE point to */
void vmprint(pagetable_t pagetable) {
    printf("page table %p\n", pagetable);
    vmprint_helpler(pagetable, 0);
}

void vmprint_helpler(pagetable_t page, int level) {
    pte_t pte;
    uint64 pa;
    ...
    for (int i = 0; i < PGSIZE / sizeof(pte); i += 1) {
        pte = page[i];
        if (pte & PTE_V) {      /* if this pte is valid */
            pa = PTE2PA(pte);
            printf("%s%d: pte %p pa %p\n", prefix[level], i, pte, pa);
            vmprint_helpler((pagetable_t) pa, level + 1);
        }
    }
}
